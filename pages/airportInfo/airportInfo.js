...
Page({
    ...
    toChannelDetail(e) {
        ...
        if(userInfo.manage != "管理员") return
        wx.navigateTo({
          url: `../channelDetail/channelDetail?itemID=${itemID}`,
        })
    },
    ...
    drawCircle(parkData, that) {
        ...
        for (let i = 0; i < len; i++) {
            ...
            ctx.arc(
                arrivePoint.x,
                arrivePoint.y - offsetCircle - smallCircleWidth,
                smallCircleWidth,
                0,
                Math.PI * 2
            );
        ...
        ctx.fillText(
            "T1蓄车场",
            centerPoint.x,
            2 * centerPoint.y - (line + offset + smallCircleWidth * 2)
        );
        ...
        ctx.fillText(
            bili1 * 0 + "",
            (line + offset + offsetCircle + smallCircleWidth * 2) * 1.45,
            centerPoint.y * 1.5
        );
        ctx.fillText(
            bili1 * 45 + "",
            line + offset + offsetCircle + smallCircleWidth * 2,
            centerPoint.y
        );
        ctx.fillText(
            bili1 * 90 + "",
            (line + offset + offsetCircle + smallCircleWidth * 2) * 1.55,
            (line + offset + offsetCircle + smallCircleWidth * 2) * 1.55
        );
        ctx.fillText(
            bili1 * 135 + "",
        ...
        ctx.fillText(
            stateStr,
            centerPoint.x,
            centerPoint.y + centerFontSize / 2
        );
        ctx.save();
        ctx.draw();
