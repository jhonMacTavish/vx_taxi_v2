        ...
        for (let i = 0; i < 4; i++) {
          switch (i) {
            case 0:
              ...
              arr[i].time = data.poolInTime ? data.poolInTime : "没有时间记录";
              break;
            case 1:
              console.log(data.poolOutTime);
              arr[i].time = data.poolOutTime ? data.poolOutTime : "没有时间记录";
              break;
            case 2:
              console.log(data.terminalInTime);
              arr[i].time = data.terminalInTime ? data.terminalInTime : "没有时间记录";
              break;
            case 3:
              console.log(data.terminalOutTime);
              arr[i].time = data.terminalOutTime ? data.terminalOutTime : "没有时间记录";
              break;
            default:
              break;
          }
